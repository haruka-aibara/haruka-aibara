...
記事・学習メモは `docs/` 配下にあります。このファイル中のコンテンツのパスは、断りがなければ `docs/` からの相対です（例: `Terraform/` は `docs/Terraform/`）。

## 鉄則：しゃべりすぎない

**回答は情報量で勝負する。文字数で埋めない。**

- 聞かれたことに答える。聞かれていない周辺情報・前置き・言い換えを足さない
- やったことの報告は結論から。経緯の実況中継をしない
- 同じことを別の言い方で繰り返さない。要約の要約を付けない
- 箇条書きや見出しは、構造があるときだけ使う。短い答えは1〜2文で返す
- 判断が要るときだけ選択肢を出す。出すなら推奨を先に1つ

記事の分量ルール（後述のスキル側）と同じ発想。読む側の時間を尊重する。
...
コンテンツは `docs/` 配下。**`docs/` 直下はフラットで、1階層目はトピック名そのもの**（例: `Terraform/`, `Kubernetes/`, `Python/`, `Amazon Web Services/`）。
...
- **新しいトピックは分類せず、トピック名のディレクトリを `docs/` 直下に作る**
- **`docs/` 直下のディレクトリ名は英語**（`Presentations/`, `Cyber Attacks/`, `AI and Machine Learning/`）。単語区切りはスペース。2階層目以降は日本語でよい（`AI and Machine Learning/生成AIパスポート/`）
...
- 目次は `docs/index.html`。GitHub API からツリーを動的に取得するのでパスのハードコードはなく、ディレクトリを増減しても手を入れる必要はない
...
スキルは **リポジトリルートの `.claude/skills/` に置く**（`docs/.claude/` ではない）。Claude Code がプロジェクトスキルを探すのはリポジトリルートだけで、サブディレクトリに置くと読み込まれない。この `AGENTS.md` をルートに置いているのも同じ理由。`~/.claude/` は devcontainer 再起動で消えるので、残すものはリポジトリ内に置く。
