...
| Lambda x2 + layer x1 | the Slack-facing frontend and the Bedrock backend; the shared layer is built from `requirements.txt` by `build_layer.py` on every plan (needs `python3` with pip where Terraform runs) |
