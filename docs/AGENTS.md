...
このリポジトリを扱う AI コーディングエージェント向けの共通ルールです。Claude Code / Codex / Gemini CLI など、どのツールもこのファイルを読むので、**指示はツール別に分けずここ1本に集約します**。
...
このリポジトリは VSCode devcontainer で動いているため、`~/.claude/` のようなホーム配下のメモリファイルはコンテナ再起動・別PC起動で消える可能性があります。**会話をまたいで残すべき情報は、ツール固有のユーザーメモリ（`~/.claude/`・`MEMORY.md` 等）ではなく、リポジトリ内のファイルに書いてください。**

- 作業ルール・判断軸・既存コンテンツ情報はこの `AGENTS.md` に追記する
- **ツール別の指示ファイル（`CLAUDE.md` 等）は作らない。** 特定ツールだけに効く注意も、そのツール名を添えてこのファイルに書く（`GEMINI.md` だけは、Gemini CLI が `AGENTS.md` を読まない場合の転送用に残してある。中身は増やさない）
