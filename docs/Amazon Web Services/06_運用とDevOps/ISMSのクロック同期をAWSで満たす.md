...
ISO 27002 8.17。自分で時計を持っているところだけが対象。
...
Amazon Time Sync は AWS が各リージョンに置く NTP サーバ。VPC 内から直接届き、SG の許可も要らない。うるう秒をスメアするので公開 NTP と混ぜない。
...
定期確認は SSM State Manager で全台に流し、結果を S3 に残す。SSM エージェントは確認の経路でしかなく、無くても同期はされる。未登録の台は確認できないので EC2 の台数と突き合わせる。
